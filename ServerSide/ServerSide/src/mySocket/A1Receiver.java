...
public void run() {
    ...
    System.out.println("A1Receiver: Set the connection successfully with "+inMsg);
    int count; 
	try {
    ...
    // close the socket
    try { socket.close(); System.out.println("A1Receiver: socket closed.");} 
    	catch (IOException ex) { System.out.println("A1Receiver: error while closing the socket."); }
}  
